...
`fastlane/README.md` is generated by Fastlane; keep durable setup notes here.
...
export APP_STORE_CONNECT_API_KEY_CONTENT="$(base64 -i /path/to/AuthKey_XXXX.p8 | tr -d '\n')"
export APP_STORE_CONNECT_API_KEY_IS_KEY_CONTENT_BASE64=true
```

Validate and upload metadata only:
...
The lane sets `precheck_include_in_app_purchases: false` because Fastlane cannot
check in-app purchases with App Store Connect API key authentication.
